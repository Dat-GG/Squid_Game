...
namespace EnhancedScrollerDemos.SuperSimpleDemo
{
    ...
    public class SimpleDemo : MonoBehaviour, IEnhancedScrollerDelegate
    {
        ...
        void Start()
        {
            ...
            scroller.scrollerSnapped = ScrollerSnapping;
            scroller.scrollerScrolled = OnScrollChange;
            scroller.scrollerScrollingChanged = ScrollerChangeState;
            scroller.scrollerTweeningChanged = ScrollerTweeningChange;

            // load in a large set of data
            LoadLargeData();
        }
        
        private void ScrollerTweeningChange(EnhancedScroller scroller, bool tweening)
        {
            Debug.Log("ScrollerTweeningChange " + tweening);
        }

        private void OnScrollChange(EnhancedScroller scroller, Vector2 val, float scrollPosition)
        {
            Debug.Log("OnScrollChange");
            //throw new NotImplementedException();
        }

        private void ScrollerSnapping(EnhancedScroller scroller, int cellIndex, int dataIndex, EnhancedScrollerCellView cellView)
        {
          
            Debug.Log("ScrollerSnapping " + dataIndex);
            
      
        }
    
        private void ScrollerChangeState(EnhancedScroller scroller, bool scrolling)
        {
            Debug.Log("ScrollerChangeState " + scrolling);
        ...
        public EnhancedScrollerCellView GetCellView(EnhancedScroller scroller, int dataIndex, int cellIndex)
        {
            ...
            // the scene hierarchy.
            cellView.name = "Cell " + dataIndex.ToString() + " " + cellIndex;
