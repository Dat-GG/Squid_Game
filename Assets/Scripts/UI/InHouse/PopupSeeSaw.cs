...
{
    public class PopupSeeSaw : PopupBase
    {
        ...
        private void OnClickBuyWithGoldBtn()
        {
            ...
            if (DataAccountPlayer.PlayerMoney.GetMoney(MoneyType.Gold) >= priceItemWithGold)
            {
                ...
                _housePopup.SeeSaw.SetActive(true);
                _housePopup.DecorSeeSawBtn.gameObject.SetActive(false);
                DataAccountPlayer.PlayerSettings.SetInHouse(_housePopup.SeeSaw.name);
                Close();
            }
            else
            {
