...
{
    public class PopupSlide : PopupBase
    {
        ...
        private void OnClickBuyWithGoldBtn()
        {
            ...
            if (DataAccountPlayer.PlayerMoney.GetMoney(MoneyType.Gold) >= priceItemWithGold)
            {
                ...
                _housePopup.Slide.SetActive(true);
                _housePopup.DecorSlideBtn.gameObject.SetActive(false);
                DataAccountPlayer.PlayerSettings.SetInHouse(_housePopup.Slide.name);
                Close();
            }
            else
            {
