...
{
    public class PopupGuard : PopupBase
    {
        ...
        private void OnClickBuyWithGoldBtn()
        {
            ...
            if (DataAccountPlayer.PlayerMoney.GetMoney(MoneyType.Gold) >= priceItemWithGold)
            {
                ...
                _housePopup.Guard.SetActive(true);
                _housePopup.DecorGuardBtn.gameObject.SetActive(false);
                DataAccountPlayer.PlayerSettings.SetInHouse(_housePopup.Guard.name);
                Close();
            }
            else
            {
