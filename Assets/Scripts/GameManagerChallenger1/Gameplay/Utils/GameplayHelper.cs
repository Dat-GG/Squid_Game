...
namespace GameManagerChallenger1.Gameplay.Utils
{
    public class GameplayHelper : MonoBehaviour
    {
        public static GameObject SpawnPlayer(Vector3 position)
        {
            var gamePrefab = LoadResourceController.Instance.LoadPlayer();
        ...
        public static GameObject SpawnDolly(Vector3 position)
        {
            var gamePrefab = LoadResourceController.Instance.LoadDolly();
            if (gamePrefab is null)
                return null;
        ...
        {
            var gamePrefab = LoadResourceController.Instance.LoadBot();
            if (gamePrefab is null)
                return null;
        ...
        {
            var gamePrefab = LoadResourceController.Instance.LoadPolice();
            if (gamePrefab is null)
                return null;
